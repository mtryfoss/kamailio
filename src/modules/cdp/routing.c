...
#define LB_MAX_PEERS \
	20 /**< maximum peers that can be loadbalanced accross i.e. same metric */
...
int peer_handles_application(peer *p, int app_id, int vendor_id)
{
	...
	if(!p || !p->applications || !p->applications_cnt)
		return 0;
	for(i = 0; i < p->applications_cnt; i++)
		if(p->applications[i].id == app_id
				&& p->applications[i].vendor == vendor_id)
			return 1;
	return 0;
}
 ...
 */
peer *get_first_connected_route(
		cdp_session_t *cdp_session, routing_entry *r, int app_id, int vendor_id)
{
	...
	LM_DBG("peer [%.*s] was last used @ %" TIME_T_FMT "\n", peers[0]->fqdn.len,
			peers[0]->fqdn.s, TIME_T_CAST(peers[0]->last_selected));
	p = peers[0];
	for(j = 1; j < peer_count; j++) {
		LM_DBG("Peer [%.*s] was last used at [%" TIME_T_FMT "]\n",
				peers[j]->fqdn.len, peers[j]->fqdn.s,
				TIME_T_CAST(peers[j]->last_selected));
	...
	p->last_selected = (time_spec.tv_sec * 1000000)
					   + round(time_spec.tv_nsec
							   / 1.0e3); // Convert nanoseconds to microseconds
