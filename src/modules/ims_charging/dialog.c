		...
		struct dlg_cell *dlg, int type, struct dlg_cb_params *_params)
{
	...
	switch(type) {
		...
		case DLGCB_TERMINATED:
			dlg_terminated(
					dlg, type, termcode, "normal call clearing", _params);
			break;
		case DLGCB_FAILED:
...
void dlg_answered(struct dlg_cell *dlg, int type, struct dlg_cb_params *_params)
{
	...
	int ret = 0;
	LM_DBG("we were granted %d seconds (valid for %d seconds) and it's been %d "
		...
		char *reason, struct dlg_cb_params *_params)
{
	...
	if(_params->req) {
		if(_params->req->first_line.u.request.method_value == METHOD_BYE) {
			...
			LM_DBG("Request is %.*s\n",
					_params->req->first_line.u.request.method.len,
	...
	if(dlg && (dlg->callid.s && dlg->callid.len > 0)) {
		...
		LM_DBG("processing dlg_terminated in Ro and session [%.*s] has active "
			   "= %d\n",
		...
		if((!ro_session->active && (ro_session->start_time != 0))
			...
			LM_DBG("CCR already sent or Ro Session is not active, but may have "
				   "been answered [%d]\n",
